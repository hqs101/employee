            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                    	<form role="form" class="form-inline" id="query-form" method="post" action="${ctx }/admin/roleMenu/list" >
                    		<!-- 查询开始 -->
                            <div class="form-group">
                                <label for="query-role" class="sr-only">角色</label>
                                	<input type="text" placeholder="角色" id="query-role" name="role" value="${query.role }" class="form-control">
                            </div>
                            <div class="form-group">
						...
						<table id="table" data-toggle="table" data-height="400" data-click-to-select="true" data-mobile-responsive="true">
							...
							<tbody>
								...
								<tr data-index="${idx.index }">
									...
									<td style="">${n.id }</td>
									<td style="">${n.role.name}</td>
									<td style="">${n.menu.name}</td>
								</tr>
								</c:forEach>
							</tbody>
						</table>
...
function add() {
		layer.open({
		    ...
		    content: "${ctx}/admin/roleMenu/detail",
		    btn: ['确定', '取消'],
...
function edit() {
	...
	layer.open({
	    ...
	    skin: 'layui-layer-rim', //加上边框
	    area: [layer_default_width, layer_default_height], //宽高
	    content: "${ctx}/admin/roleMenu/detail?id=" + id,
	    btn: ['确定', '取消'],
	    yes: function(index, layero){ //或者使用btn1
	    	var detailForm = layer.getChildFrame('form', index);
	    	console.log(detailForm);
